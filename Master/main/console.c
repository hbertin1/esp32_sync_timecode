/*
    Command line controls over the USB serial interface
*/
...
/* request LTC from the paired device, NOT IMPLEMENTED yet */
static int reqLTC()
{
    ESP_LOGI(CLI_TAG, "Requesting LTC from the master...");
    uint8_t code[1];
    code[0] = REQUEST_LTC_CODE;
    /* Store the request time */
    gettimeofday(&latency.time_send_request, NULL);
    esp_spp_write(129, 1, code);
    return 0;
}
...
void register_cmd(void)
{
    ...
    const esp_console_cmd_t reqLTC_cmd = {
        .command = "reqLTC",
        .help = "request LTC from the master device",
        .hint = NULL,
        .func = &reqLTC,
        .argtable = NULL
    };

    ESP_ERROR_CHECK(esp_console_cmd_register(&reqLTC_cmd));
}
